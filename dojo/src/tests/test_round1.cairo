...
mod tests {
    ...
    fn _start_new_challenge(sys: tester::TesterSys, owner: ContractAddress, other: ContractAddress, wager_value: u256) -> (Challenge, Round, u128) {
        ...
        let duel_id: u128 = tester::execute_create_challenge(sys, sys.owner, sys.other, MESSAGE_1, TABLE_ID, wager_value, expire_seconds);
        ...
        let round: Round = tester::get_Round(sys, duel_id, 1);
    ...
    fn test_challenge_accept_state() {
        ...
        let new_state: ChallengeState = tester::execute_reply_challenge(sys, sys.other, duel_id, true);
    ...
    fn test_resolved() {
        let sys = tester::setup_world_sys(true, true);
        let balance_contract: u256 = sys.ierc20.balance_of(sys.actions.contract_address);
        let balance_treasury: u256 = sys.ierc20.balance_of(sys.treasury);
        let balance_a: u256 = sys.ierc20.balance_of(sys.owner);
        let balance_b: u256 = sys.ierc20.balance_of(sys.other);
        let fee: u256 = sys.actions.calc_fee(TABLE_ID, WAGER_VALUE);
        assert(fee > 0, 'fee > 0');
        assert(balance_treasury == 0, 'balance_treasury == 0');

        let (challenge, _round, duel_id) = _start_new_challenge(sys, sys.owner, sys.other, WAGER_VALUE);
        tester::assert_balance(sys, sys.actions.contract_address, balance_contract, 0, (fee + WAGER_VALUE) * 2, 'balance_contract_1');
        tester::assert_balance(sys, sys.owner, balance_a, fee + WAGER_VALUE, 0, 'balance_a_1');
        tester::assert_balance(sys, sys.other, balance_b, fee + WAGER_VALUE, 0, 'balance_b_1');
        tester::assert_balance(sys, sys.treasury, 0, 0, 0, 'balance_treasury_1');

        let (salt_a, salt_b, action_a, action_b, hash_a, hash_b) = _get_actions_round_1_resolved();
        let _hit_chance_a = sys.actions.simulate_chances(sys.owner, challenge.duel_id, challenge.round_number, action_a).hit_chances;
        let _hit_chance_b = sys.actions.simulate_chances(sys.owner, challenge.duel_id, challenge.round_number, action_b).hit_chances;
        let kill_chance_a = sys.actions.simulate_chances(sys.owner, challenge.duel_id, challenge.round_number, action_a).hit_chances;
        let kill_chance_b = sys.actions.simulate_chances(sys.owner, challenge.duel_id, challenge.round_number, action_b).hit_chances;

        // 1st commit
        tester::execute_commit_action(sys, sys.owner, duel_id, 1, hash_a);
        ...
        // 2nd commit > Reveal
        tester::execute_commit_action(sys, sys.other, duel_id, 1, hash_b);
        let (challenge, round) = tester::get_Challenge_Round(sys, duel_id);
        ...
        // 1st reveal
        tester::execute_reveal_action(sys, sys.owner, duel_id, 1, salt_a, action_a, 0);
        let (challenge, round) = tester::get_Challenge_Round(sys, duel_id);
        ...
        // 2nd reveal > Finished
        tester::execute_reveal_action(sys, sys.other, duel_id, 1, salt_b, action_b, 0);
        let (challenge, round) = tester::get_Challenge_Round(sys, duel_id);
...
// challenge.state.print();
        ...
        let duelist_a = tester::get_Duelist(sys, sys.owner);
        let duelist_b = tester::get_Duelist(sys, sys.other);
        ...
        tester::assert_balance(sys, sys.actions.contract_address, balance_contract, 0, 0, 'balance_contract_2');
        let balance_treasury = tester::assert_balance(sys, sys.treasury, balance_treasury, 0, fee * 2, 'balance_treasury_2');
        tester::assert_winner_balance(sys, challenge.winner, sys.owner, sys.other, balance_a, balance_b, fee, WAGER_VALUE, 'balance_winner_2');
        let balance_a: u256 = sys.ierc20.balance_of(sys.owner);
        let balance_b: u256 = sys.ierc20.balance_of(sys.other);

        // Run same challenge to compute totals
        let (_challenge, _round, duel_id) = _start_new_challenge(sys, sys.owner, sys.other, WAGER_VALUE);
        ...
        tester::assert_balance(sys, sys.actions.contract_address, balance_contract, 0, 0, 'balance_contract_3');
        tester::assert_balance(sys, sys.treasury, balance_treasury, 0, fee * 2, 'balance_treasury_3');
        tester::assert_winner_balance(sys, challenge.winner, sys.owner, sys.other, balance_a, balance_b, fee, WAGER_VALUE, 'balance_winner_3');

        // Snapshot was created and kept original value
        let snapshot = tester::get_Snapshot(sys, duel_id);
...
// challenge.state.print();
        ...
        let mut scoreboard_a = tester::get_Scoreboard(sys, sys.owner, TABLE_ID);
        let mut scoreboard_b = tester::get_Scoreboard(sys, sys.other, TABLE_ID);
        ...
        tester::assert_balance(sys, sys.actions.contract_address, balance_contract, 0, 0, 'balance_contract_2');
        tester::assert_balance(sys, sys.treasury, 0, 0, fee * 2, 'balance_treasury_2');
        tester::assert_balance(sys, sys.owner, balance_a, fee, 0, 'balance_a_2');
        tester::assert_balance(sys, sys.other, balance_b, fee, 0, 'balance_b_2');
    }
    ...
    fn test_dual_crit_to_trickster_a() {
        ...
        // let mut duelist_a = tester::get_Duelist(sys, sys.owner);
        // duelist_a.score.level_trickster = 100;
        let mut scoreboard_a = tester::get_Scoreboard(sys, sys.owner, TABLE_ID);
        scoreboard_a.score.level_trickster = 100;
        set!(sys.world,(scoreboard_a));
        // duel!
        let (_challenge, _round, duel_id) = _start_new_challenge(sys, sys.owner, sys.other, WAGER_VALUE);
        let (salt_a, salt_b, action_a, action_b, hash_a, hash_b) = _get_actions_round_1_dual_crit(10, 10);
        tester::execute_commit_action(sys, sys.owner, duel_id, 1, hash_a);
    ...
    fn test_dual_crit_to_trickster_b() {
        let sys = tester::setup_world_sys(true, true);
        // A is a trickster, will shoot first
        // let mut duelist_b = tester::get_Duelist(sys, sys.other);
        // duelist_b.score.level_trickster = 100;
        let mut scoreboard_b = tester::get_Scoreboard(sys, sys.other, TABLE_ID);
        scoreboard_b.score.level_trickster = 100;
        set!(sys.world,(scoreboard_b));
        // duel!
        let (_challenge, _round, duel_id) = _start_new_challenge(sys, sys.owner, sys.other, WAGER_VALUE);
        let (salt_a, salt_b, action_a, action_b, hash_a, hash_b) = _get_actions_round_1_dual_crit(10, 10);
        tester::execute_commit_action(sys, sys.owner, duel_id, 1, hash_a);
    ...
    fn test_dual_hit() {
        ...
        let round1: Round = tester::get_Round(sys, challenge.duel_id, 1);
...
// round1.shot_b.chance_hit.print();
    ...
    fn test_clamp_invalid_paces() {
        let sys = tester::setup_world_sys(true, true);
        let (_challenge, _round, duel_id) = _start_new_challenge(sys, sys.owner, sys.other, 0);
        let hash_a: u64 = make_action_hash(0x111, 0);
        let hash_b: u64 = make_action_hash(0x222, 11);
        tester::execute_commit_action(sys, sys.owner, duel_id, 1, hash_a);
        tester::execute_commit_action(sys, sys.other, duel_id, 1, hash_b);
        tester::execute_reveal_action(sys, sys.owner, duel_id, 1, 0x111, 0, 0);
        tester::execute_reveal_action(sys, sys.other, duel_id, 1, 0x222, 11, 0);
        let round: Round = tester::get_Round(sys, duel_id, 1);
    ...
    fn test_register_keep_scores() {
        ...
        tester::execute_commit_action(sys, sys.owner, duel_id, 1, hash_a);
        tester::execute_commit_action(sys, sys.other, duel_id, 1, hash_b);
        tester::execute_reveal_action(sys, sys.owner, duel_id, 1, 0x111, 10, 0);
        let duelist_a_before = tester::get_Duelist(sys, sys.owner);
        tester::execute_register_duelist(sys, sys.owner, 'dssadsa', 3);
        let duelist_a_after = tester::get_Duelist(sys, sys.owner);
    ...
    fn test_wrong_player() {
        ...
        tester::execute_commit_action(sys, someone_else, duel_id, 1, hash);
    }
    ...
    fn test_wrong_round_number() {
        ...
        let (_challenge, _round, duel_id) = _start_new_challenge(sys, sys.owner, sys.other, 0);
        let hash: u64 = make_action_hash(0x12121, 0x1);
        tester::execute_commit_action(sys, sys.owner, duel_id, 2, hash);
    }
    ...
    fn test_invalid_hash_action_a() {
        ...
        let hash_b: u64 = make_action_hash(0x222, 1);
        tester::execute_commit_action(sys, sys.other, duel_id, 1, hash_a);
        tester::execute_commit_action(sys, sys.owner, duel_id, 1, hash_b);
        tester::execute_reveal_action(sys, sys.owner, duel_id, 1, 0x111, 2, 0);
    }
    #[test]
    #[available_gas(1_000_000_000)]
    #[should_panic(expected:('Action does not match hash','ENTRYPOINT_FAILED'))]
    fn test_invalid_hash_salt_a() {
        ...
        tester::execute_commit_action(sys, sys.owner, duel_id, 1, hash_b);
        tester::execute_commit_action(sys, sys.other, duel_id, 1, hash_a);
        tester::execute_reveal_action(sys, sys.owner, duel_id, 1, 0x1111, 1, 0);
    }
    ...
    fn test_invalid_hash_action_b() {
        ...
        tester::execute_commit_action(sys, sys.owner, duel_id, 1, hash_b);
        tester::execute_commit_action(sys, sys.other, duel_id, 1, hash_a);
        tester::execute_reveal_action(sys, sys.other, duel_id, 1, 0x222, 2, 0);
    }
    #[test]
    #[available_gas(1_000_000_000)]
    #[should_panic(expected:('Action does not match hash','ENTRYPOINT_FAILED'))]
    fn test_invalid_hash_salt_b() {
        ...
        tester::execute_commit_action(sys, sys.owner, duel_id, 1, hash_b);
        tester::execute_commit_action(sys, sys.other, duel_id, 1, hash_a);
        tester::execute_reveal_action(sys, sys.other, duel_id, 1, 0x2222, 1, 0);
    }
